...
import {
  ...
  CheckCircle, AlertCircle, Loader2, Save, ArrowLeft,
} from "lucide-react";
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer group">
      <div
        ...
        className={`relative w-10 rounded-full transition-colors flex-shrink-0 ${checked ? "bg-[#0d7434]" : "bg-slate-200"}`}
        style={{ height: "22px" }}
      >
...
export default function EditPackagePage() {
  ...
  const [form, setForm]               = useState<AdminCreatePackagePayload | null>(null);
    ...
    adminApi.getPackage(id)
      .then(res => {
        ...
        setForm({
          ...
          category:             d.category  ?? "umrah",
          package_type:         d.package_type ?? "economy",
          status:               d.status    ?? "draft",
          seasonal_tag:         d.seasonal_tag ?? "",
          duration_days:        d.duration_days ?? 7,
          duration_nights:      d.duration_nights ?? 0,
          start_date:           d.start_date  ?? null,
          end_date:             d.end_date    ?? null,
          flexible_dates:       d.flexible_dates ?? false,
          makkah_nights:        d.makkah_nights  ?? null,
          madinah_nights:       d.madinah_nights ?? null,
          base_price:           d.base_price  ?? 0,
          currency:             d.currency    ?? "USD",
          price_per_adult:      d.price_per_adult  ?? null,
          price_per_child:      d.price_per_child  ?? null,
          price_per_infant:     d.price_per_infant ?? null,
          discount_type:        d.discount_type  ?? null,
          discount_value:       d.discount_value ?? null,
          installment_option:   d.installment_option ?? false,
          airline_name:         d.airline_name ?? "",
          flight_type:          d.flight_type  ?? "direct",
          flight_class:         d.flight_class ?? "economy",
          departure_city:       d.departure_city ?? "",
          airport_transfers:    d.airport_transfers ?? false,
          intercity_transport:  d.intercity_transport ?? "",
          includes_flights:     d.includes_flights ?? false,
          includes_visa:        d.includes_visa    ?? false,
          meal_plan:            d.meal_plan         ?? "",
          ziyarat_included:     d.ziyarat_included  ?? false,
          laundry_service:      d.laundry_service   ?? false,
          wheelchair_assistance: d.wheelchair_assistance ?? false,
          ihram_provided:       d.ihram_provided    ?? false,
          language_spoken:      d.language_spoken   ?? [],
          guide_languages:      d.guide_languages   ?? [],
          services_included:    d.services_included ?? [],
          services_excluded:    d.services_excluded ?? [],
          min_group_size:       d.min_group_size  ?? 1,
          max_group_size:       d.max_group_size  ?? null,
          max_persons:          d.max_persons     ?? null,
          available_slots:      d.available_slots ?? null,
          ...
          hotels:               (d.hotels ?? []) as HotelEntryPayload[],
          itinerary:            (d.itinerary ?? []) as ItineraryDayPayload[],
        });
      })
  ...
  const handleTitleEn = (v: string) => {
    set("title_en", v);
    if (form && (!form.slug || form.slug === slugify(form.title_en ?? ""))) {
      set("slug", slugify(v));
    }
  };
  ...
  const validate = () => {
    if (!form) return { _global: "Form not loaded" };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <Loader2 size={24} className="animate-spin text-[#0d7434]" />
      </div>
    );
  }
  ...
  if (notFound || !form) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <p className="text-slate-600 font-medium">Package not found</p>
        <button onClick={() => router.push(`/${locale}/admin/packages`)}
          className="text-sm text-[#0d7434] font-semibold hover:underline">
          ← Back to Packages
        </button>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 style={{ fontSize: "20px", fontWeight: 800, color: "#0f1416" }}>Edit Package</h1>
            <p className="text-sm text-slate-500 mt-0.5">Update details across all sections</p>
          </div>
        </div>
      ...
      <AnimatePresence>
        {saveStatus === "saved" && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <CheckCircle size={15} /> Package updated successfully. Redirecting…
          </motion.div>
        )}
            ...
            <motion.div key={section} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.15 }}>
              ...
              {section === "general" && (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Short Summary (English)">
                      <textarea rows={2} value={form.summary_en ?? ""} onChange={e => set("summary_en", e.target.value)}
                        className={textareaCls} placeholder="Brief overview…" />
                    </Field>
                  ...
                  <Toggle checked={form.flexible_dates ?? false} onChange={v => set("flexible_dates", v)} label="Flexible Dates" />
                    ...
                    <div key={i} className="border border-slate-200 rounded-2xl p-5 relative">
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        <Field label="Hotel Name (English)" required>
                          <input type="text" value={h.hotel_name} onChange={e => setHotel(i, { hotel_name: e.target.value })} className={inputCls} placeholder="e.g. Hilton Makkah" />
                        </Field>
              ...
              {section === "media" && (
                <div className="flex flex-col gap-4">
                  ...
                  <Field label="Video URL" hint="Optional — YouTube / Vimeo embed or direct link">
