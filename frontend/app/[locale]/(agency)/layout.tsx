...
const NAV_KEYS = [
  { key: "dashboard", icon: LayoutDashboard, path: "/agency/dashboard" },
  { key: "packages",  icon: Package,         path: "/agency/packages"  },
  { key: "bookings",  icon: ShoppingBag,     path: "/agency/bookings"  },
  { key: "reviews",   icon: Star,            path: "/agency/reviews"   },
  { key: "settings",  icon: Settings,        path: "/agency/settings"  },
];
    ...
    >
      ...
      <div className="flex flex-col items-center pt-7 pb-5 px-4">
        ...
        <p style={{ color: "#fff", fontSize: "14px", fontWeight: 700, letterSpacing: "0.02em" }}>
          {user ? `${user.first_name} ${user.last_name}` : "Agency Owner"}
        </p>
      ...
      <nav className="flex-1 px-3 overflow-y-auto" style={{ gap: "2px", display: "flex", flexDirection: "column" }}>
        {NAV.map(({ label, icon: Icon, href }) => {
          const active = pathname === href || (href !== `/${locale}/agency/dashboard` && pathname.startsWith(href));
          return (
            <Link
              key={href}
              href={href}
            ...
            >
              <Icon size={17} style={{ flexShrink: 0, color: active ? "#1abc9c" : "rgba(255,255,255,0.5)" }} />
              {label}
            </Link>
          );
        })}
      ...
      <div className="px-3 pb-3 mt-3">
        <button
          onClick={async () => {
            ...
            setUserStore(null);
            router.replace(`/${locale}/agency/login`);
          }}
...
export default function AgencyLayout({ children }: { children: React.ReactNode }) {
  ...
  const tNav = useTranslations("agency.nav");
  const tHeader = useTranslations("agency.header");
  const user = useAuthStore((s) => s.user);
  ...
  useEffect(() => {
    auth.me()
      ...
      .catch(() => router.replace(`/${locale}/agency/login`));
  }, []);
  ...
  return (
    <div className="flex min-h-screen relative" style={{ backgroundColor: "#dff0e9" }}>
      ...
      <AirplanePlaceholder
        variant="left"
        style={{ position: "fixed", left: "-55px", top: "32%", width: "220px", height: "140px", zIndex: 35, opacity: 0.82, pointerEvents: "none", userSelect: "none" }}
      />
      <AirplanePlaceholder
        variant="right"
        style={{ position: "fixed", right: "-18px", bottom: "-18px", width: "280px", height: "175px", zIndex: 5, opacity: 0.78, pointerEvents: "none", userSelect: "none" }}
      />
      ...
      {[
        ...
        <div
          ...
          style={{ width: d.size, height: d.size, backgroundColor: "#0d7434", opacity: d.op, top: d.top, left: (d as any).left, right: (d as any).right, zIndex: 4 }}
        />
      ))}
        ...
        >
          ...
          <p className="hidden lg:block font-bold text-gray-800" style={{ fontSize: "16px" }}>
            {NAV_RESOLVED.find(n => n.href === pathname)?.label
              ?? NAV_RESOLVED.find(n => pathname.startsWith(n.href) && n.href !== `/${locale}/agency/dashboard`)?.label
              ?? tNav("dashboard")}
          </p>
          ...
          <div className="flex items-center gap-3 ms-auto">
            ...
            <div className="flex items-center gap-0.5 bg-white/60 rounded-xl p-1">
              {(["en", "ar"] as const).map((lng) => (
                <Link
                  ...
                  style={{ backgroundColor: locale === lng ? "#0d7434" : "transparent", color: locale === lng ? "#fff" : "#6b7280" }}
                >
                  {lng === "en" ? "EN" : "ع"}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-2 bg-white/60 rounded-xl px-2.5 py-1.5">
              <div
                ...
                style={{ width: "32px", height: "32px", background: "#0d7434", color: "#fff", fontWeight: 700, fontSize: "14px" }}
              >
                {user?.first_name?.[0]?.toUpperCase() ?? "A"}
              </div>
              <div className="hidden sm:block">
                <p style={{ fontSize: "12px", fontWeight: 700, color: "#1a1a1a" }}>{tHeader("agencyOwner")}</p>
                <p style={{ fontSize: "10px", color: "#a6aaac" }}>{user?.email ?? ""}</p>
              </div>
            </div>
          </div>
        </header>
